...
1. 使用npm安裝`mcp-remote` 套件。

```
npm i mcp-remote
```
2. 開啟Claude Desktop MCP設定檔。

```markdown
%APPDATA%\Roaming\Claude\claude_desktop_config.json
```

3. 調整Claude設定檔，加入MCP服務運作設定。
...
4. 若Claude Desktop應用程式在修改設定檔之前已經開啟，則需在設定檔調整完之後至桌面右下角點選Claude Desktop圖示，右鍵選擇Quit完整關閉Claude Desktop並重新開啟應用程式。
...
5. 重啟Claude Desktop後，開啟Settings→Developer，確認成功引入tdx_rail_http服務且狀態為running，並確認TDX API金鑰(TDX_CLIENT_ID和TDX_CLIENT_SECRET)資訊是否正確。

![image.png](images/image_1.png)

6. 回到提問視窗，點選左下角「+」圖示→Connector→Add from tdx_rail→挑選Tdx basic prompt，視需求使用預先定義好的Prompt，並確認tdx_rail_http為開啟狀態。

![image.png](images/image_2.png)

7. 點選「Manage connectors」，開啟Connectors設定，點選「tdx_rail_http」的Configure按鈕。確認所有MCP工具的狀態為「Always allow」或「Needs approval」。
